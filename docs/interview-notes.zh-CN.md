...
这是一个参考 Open Deep Research / Deep Research 类系统自己实现的 **Conversational Research Agent Runtime 实验项目**。它先用 chat session 收集团队约束和研究目标，生成需要用户确认的研究计划，再由 supervisor 分派研究单元，自动选择 Web Search、GitHub MCP 和本地 Agentic RAG 等证据源，最后生成带引用、可验证、可复盘的技术调研报告。

更诚实的说法是：

> 我不是在做一个要打败 Codex 的商业 Copilot，而是在学习并复现复杂研究 Agent 背后的工程机制：session state、memory、interactive planning、LangGraph 状态流、结构化 tool call、多证据路由、RAG grounding、citation synthesis、verifier/evaluator 和 trace replay。

## 面试官问“用 Codex 不就好了？”

可以这样回答：

> 对，作为最终用户工具，Codex / Deep Research 当然更强。我这个项目不是想替代它，而是把这类系统背后的 agent runtime 拆出来自己实现了一遍。重点在工程机制：怎么维护 session 和 memory，怎么先让用户确认计划，怎么让 supervisor 做结构化工具决策，怎么把 Web、GitHub MCP、本地 RAG 都统一成证据，怎么约束引用，怎么评估 groundedness，怎么保存 trace 并 replay 一次运行。

这比说“我做了一个更好的研究助手”稳得多。

## 下一步还需要打磨什么

session、memory、计划确认和 workbench 已经补上了。现在最应该继续打磨的是面试可展示的硬证据：
...
- `/v1/memory`
- SQLite memory
- 研究前 plan confirmation
- 静态 Agent Workbench

但边界要讲清楚：

- 这是本地单用户 memory，不是企业级长期个性化系统。
- memory extractor v1 是轻量规则，不是成熟记忆产品。
- 本地 self-calling workbench MCP server 仍然删除，因为它会让 MCP 边界变混乱。
- GitHub MCP 是外部工具证据源，不是让别的 deep research agent 替你完成主流程。

现在保留并强化的主链路是：会话、记忆、确认计划、规划、监督、检索、证据、报告、验证、评估、trace。
...
- `schemas.py` 里的结构化契约。
- `agent.py` 里的 session、memory、plan confirmation。
- `providers.py` 里的真模型调用。
- `graph_runtime.py` 里的 LangGraph 状态图。
- `pipeline.py` 里的应用装配和跨模块编排。
- `apps/web/index.html` 里的 Agent Workbench。
...
- SQLite：保存 session、message、plan draft、memory、job、run 和 trace。
- Qdrant：负责向量检索。
...
- session state、memory 和 human confirmation。
...
> Conversational Research Agent Runtime：面向复杂技术调研的多证据 Agent 实验系统

推荐 bullet：

> 基于 FastAPI、LangGraph、OpenAI-compatible Provider、Qdrant、SQLite FTS5/BM25、Reranker 和 SQLite memory 实现 Conversational Research Agent Runtime，支持 session state、计划确认、复杂问题规划、ODR-style supervisor tool call、Web/GitHub MCP/本地文档多证据路由、引用报告生成、验证评估与 trace replay。
...
- 把 memory 限定在本地 agent/session 层，同时保留清晰的外部 MCP 工具边界，避免噱头模块影响项目可信度。
